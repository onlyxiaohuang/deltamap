...
- 独立复盘时间线：记录每个时间点的攻守方步兵、载具、据点与关键事件，并自动计算兵力增减
- 完整离线资源
...
左侧“复盘”页可以按回合建立时间线。填写进攻方、防守方，在每个观察时间点记录双方当前可确认的步兵和载具数量、所在据点以及主攻方向等事件；后续节点会自动显示相对上一节点的数量变化。复盘数据和地图绘图保存在同一个战术 JSON 中。
