...
- 独立复盘播放器：保存地图上的步兵与载具分布快照，通过进度条回看任意时间的双方位置和兵力增减
- 完整离线资源
...
左侧“复盘”页可以按回合建立分布时间线。先选择攻守双方对应颜色，在地图上摆好该时刻可确认的步兵与载具图标，再点击“保存当前兵力分布”。拖动进度条时，地图会显示不晚于当前时间的最近快照，并自动统计双方步兵、载具数量及相对上一快照的增减。点击“退出播放”即可回到地图编辑。复盘快照和地图绘图保存在同一个战术 JSON 中。
